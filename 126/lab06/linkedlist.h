...
template <class T> int linkedlist<T>::testMax(const int &newMax) {
...
template <class T> int linkedlist<T>::listSize() const { return size; }

template <class T> int linkedlist<T>::maxListsize() const { return max_size; }
...
template <class T> void linkedlist<T>::insertAt(int index, T element) {
  if (index < 0 || index > size) {
...
template <class T> void linkedlist<T>::insertEnd(T element) {
...
template <class T> int linkedlist<T>::retrieveAt(int index) {
  Node *nodePtr = nullptr;
...
template <class T> void linkedlist<T>::replaceAt(int index, T element) {
  Node *nodePtr = nullptr;
  if (index < 0 || index > size) {
    throw std::out_of_range(
...
template <class T> void linkedlist<T>::extend(const linkedlist *list) {
  if (list->isEmpty()) {
...
template <class T> T linkedlist<T>::min() const {
  Node *nodePtr = head;
  T min;
  if (isEmpty()) {
...
template <class T> T linkedlist<T>::max() const {
  Node *nodePtr = head;
  T max = nodePtr->data;
  if (isEmpty()) {
    throw std::out_of_range("NO MIN: List is empty!");
...
template <class T> int linkedlist<T>::count(const T &element) {
  int count = 0;
  Node *nodePtr = head;
  while (nodePtr != nullptr) {
